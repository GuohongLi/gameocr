...
def readWordList(filename):
    ...
    with open(filename) as fp:
        for line in fp:
            ...
            if len(tokens) != 2:
                continue
            wordlist.append((tokens[0], tokens[1]))
    return wordlist
...
def GenCh(f,val,size, color):
    ...
    draw.text((1, -1),val,fill=color,font=f)
    A = np.array(img)
    return A
...
def GenEng(f,val,size, color):
    img=Image.new("RGB", size,(0,0,0))
    draw = ImageDraw.Draw(img)
    draw.text((1, -1),val.decode('utf-8'),color,font=f)
    A = np.array(img)
    return A
...
class GenWord:
    ...
    def __init__(self,fontCh,fontEng,BackGrounds,colorfile,wordsfile, imgsize):
        self.fontC =  ImageFont.truetype(fontCh,10,0);
        self.fontE =  ImageFont.truetype(fontEng,10,0);
    ...
    def genWordString(self,val,wordset, maxrange):
        ...
        if val == "":
            index = r(maxrange)
            wordStr = wordset[index][1]
            wordid = wordset[index][0]
        else:
            for id,w in wordset:
                if w == val:
                    wordStr = val
        ...
        return wordStr,wordid

    def genBatch(self, batchSize,wordset,colors,outputPath,size):
        if (not os.path.exists(outputPath)):
            os.mkdir(outputPath)
        for i in xrange(batchSize):
                wordStr,wordid = self.genWordString("",wordset, len(wordset))
                img =  self.generate(wordStr,colors);
                img = cv2.resize(img,size);
                cv2.imshow("final",img)
                cv2.waitKey(0)
                cv2.imwrite(outputPath + "/" + "%s_%s_%s" % (wordid,wordStr,i) + ".jpg", img);
                
    def genBatchByIteration(self, iternum, colornum, wordset,colors,outputPath,size):
        if (not os.path.exists(outputPath)):
            os.mkdir(outputPath)
        for i in xrange(iternum):
            print "iter:",i
            for wordid,wordStr in wordset:
                #wordStr,wordid = self.genWordString(word,wordset, len(wordset))
                for cn in range(colornum):
                    img =  self.generate(wordStr,colors);
                    img = cv2.resize(img,size);
                    #cv2.imshow("final",img)
                    #cv2.waitKey(0)
                    cv2.imwrite(outputPath + "/" + "%s_%s_%s_%s"%(wordid,wordStr,i,cn) + ".jpg", img);

G = GenWord("./fonts/msyh.ttf",'./fonts/msyh.ttf',"./background","./color/color.txt", "./words/words.txt",(12,12))
G.genBatchByIteration(25,4,G.words,G.colors,"./sample",(12,12))
